    ...
    for x in range(img.shape[0]):
        for y in range(img.shape[1]):
            if x < r or y < r or (x >= (img.shape[0]-r)) or (y >= (img.shape[1]-r)):
                ...
                for i in range(0, rad_range):
                    for j in range(0, rad_range):
                        window[i][j] = img[x +i -r, y +j -r]
